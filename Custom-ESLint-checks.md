...
To get a better idea for how lint checks work, write a simple ESLint rule that forbids the string `Hello, world!`. Maybe we don't want people accidentally opening PRs that include code from when they were learning JavaScript. First consider what kinds of code should violate the rule. Here are some examples:
...
We could write a rule that raises an error whenever it finds a node that:
...
For writing rules, enable "Transform" on AST Explorer.
...
To add the rule to Oppia's suite of lint checks, we need to add some more metadata describing each failure. For our hello world rule, we would create a JavaScript file in `scripts/linters/custom_eslint_checks/rules/` with the following code:
...
2. Run `yarn install` to install your linter.

3. Execute the pre-commit linter, which will execute your new rule. To speed up the lint checks, we recommend running the rule on just a single file when testing it. If you created a `test.js` file with some code to try linting, you would run the linter like this:
